jnr = ["@jcunniet","@PaulLampon","@Aziliz31","@ssoumier","@marionsouzeau","@gaellombart","@bendarag","@AurelieLebelle","@julienduffe","@thomaspoupeau","@LilyRossignol","@ClairGuedon",
    ...
    print "> Il y a "
    print jnr.count { |w| w.length == 6 } 
    puts " handles contenant 5 caractères."
    puts ""

    puts "Combien commencent par une majuscule ?"
    print ">  Il y a " 
    print jnr.count  {|e| e =~ /^@[A-Z]/}
    puts " handles qui commencent par une majuscule."
    puts ""

    puts "Voici la liste en ordre alphabétique"
    puts jnr.sort
    puts ""


    puts "Listé trié par taille des handles"
    puts jnr.sort {|x, y| x.length <=> y.length}
    puts ""


    puts "Quelle est la position dans l'array de la personne @epenser ?"
    print "> @epenser est " 
    print jnr.index('@epenser') 
    print " ème."
    puts""
